...
class pySaliencyMap:
    ...
    def CFMGetFM(self, R, G, B):
        ...
        RG[RG < 0] = 0
        BY[BY < 0] = 0
        # obtain feature maps in the same way as intensit
        RGFM = self.FMGaussianPyrCSD(RG)
        BYRM = self.FMGaussianPyrCSD(BY)
        # return
        return RGFM, BYRM
    ## color feature maps - IttiKochOriginal
    def CFMGetFM_original(self, R, G, B):
        # max(R,G,B)
        tmp1 = cv2.max(R, G)
        RGBMax = cv2.max(B, tmp1)
    ...
    ## didn't add color and orientation version
    def SMGetSM_original(self, src):
        # difinitions step by step
        size = src.shape
        width = size[1]
        height = size[0]
        ...
        # extract feature maps
        IFM = self.IFMGetFM(I)
        CFM_RG, CFM_BY = self.CFMGetFM_original(R, G, B)
        OFM = self.OFMGetFM(I)
        # Consoicuity maps
        ICM = self.ICMGetCM(IFM)
        CCM = self.CCMGetCM(CFM_RG, CFM_BY)
